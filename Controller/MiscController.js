...
const NewsletterSubscriptionHandler = asyncHandler(async (req, res) => {
    try {
        ...
        if (!emailId) {
            logger.warn("Subscription failed: emailId is empty or null");
        ...
        const newsletterSubscriptionUser = new NewsletterSubscriptionUser({ emailId });
        await newsletterSubscriptionUser.save();
        
        logger.info(`User with emailId: ${emailId} successfully subscribed to the newsletter.`);
        res.status(200).json({ message: "Subscription successful" });

    } catch (error) {
        logger.error(`Error while subscribing user with emailId: ${req.body.emailId}: ${error.message}`);
