...
async function getLocationsFromGooglePlaces(inputLocation) {
  try {
    logger.info(
      `Request received to fetch locations for input: ${inputLocation}`
    );
...
const getLocationsAsCityAndState = (locations) => {
  return locations.map((location) => {
    const [firstWord, secondWord] = location.description
      ...
      .map((word) => word.trim());
    logger.info(
      `Transforming location: ${location.description} into city and state.`
    );
    return {
      city: firstWord || "",
      state: secondWord || "",
...
const getLocationByNameHandler = asyncHandler(async (req, res) => {
  try {
    ...
    logger.info(
      `Successfully sent transformed locations for input: ${inputLocation}`
    );
  } catch (error) {
